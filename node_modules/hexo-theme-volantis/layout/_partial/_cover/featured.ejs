<div class='cover-body'>
  <div class='top'>
    <% if (theme.cover.logo) { %>
      <img no-lazy class='logo' src='<%- url_for(theme.cover.logo) %>' alt="logo"/>
    <% } %>
        ...
        <% if (theme.cover.features) { %>
          <% getList(theme.cover.features).forEach(function(value){ %>
            <a href="<%= url_for(value.url) %>"
              ...
              <% if (value.img) { %><img alt="icon" src='<%= value.img %>'><% } else if (value.icon) { %><i class='<%= value.icon %> fa-fw'></i><% } %><p><%- value.name %></p>
            </a>
          <%})%>
        <% } %>
      </div>
