  ...
  <footer class="footer clearfix"  itemscope itemtype="http://schema.org/WPFooter">
    ...
    <% layout.forEach(function(item){ %>
          ...
          <% getList(theme.site_footer.social).forEach(function(value){ %>
            <% if (value.url && (value.icon || value.img || value.avatar)) { %>
              <a href="<%= url_for(value.url) %>"
                ...
                target="_blank"
                rel="external" itemprop="url">
                <% if (value.img) { %>
                  <img alt="img" src="<%- value.img %>"/>
                <% } else if (value.avatar) { %>
                  <img alt="img" src="<%- value.avatar %>" style="border-radius:120px"/>
                <% } %>
              </a>
            <% } %>
          <% }) %>
      ...
      <% } else if (item == 'copyright') { %>
        ...
        class="underline"
         href="https://orcid.org/0009-0001-1431-9309"
         target="orcid.widget"
         ...
         <img
            ...
            alt="ORCID iD icon"/>
          https://orcid.org/0009-0001-1431-9309
        </a>
        <p><a href="https://beian.miit.gov.cn/" target="_blank">桂ICP备2025056585号-1</a></p>
        </div>
      <% } else { %>
