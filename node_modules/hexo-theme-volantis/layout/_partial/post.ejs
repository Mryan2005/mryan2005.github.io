<div class="post post-v3 white-box reveal <%- theme.custom_css.body.effect.join(' ') %>"  itemscope itemtype="http://schema.org/Article" >
  ...
  <% if (showTitle) { %>
    ...
    <h2 class="article-title" <%- pinTitle ? 'pin' : '' %> itemprop="name headline">
      <a href="<%- url_for(post.link || post.path) %>" rel="dofollow" itemprop="url">
    ...
    <div class='meta-v3' line_style='<%- theme.article.preview.line_style %>'>
      <div>
        <% if (theme.article.preview.author) { %>
          ...
          %>
          <a class='avatar' href='<%- author.url %>'><img alt="avatar" src='<%- author.avatar %>'></a>
        ...
        <% if (post.categories && post.categories.length > 0) { %>
          ...
          <% for(cat of post.categories.toArray()){ %>
            <span hidden itemprop="about" itemscope itemtype="http://schema.org/Thing">
              <a rel="dofollow" href="<%- url_for(cat.path, {relative: false}) %>" itemprop="url"><span itemprop="name"><%- cat.name %></span></a>
            </span>
          <% } %>
        <% } %>
      </div>
      <% if (showReadmore) { %>
        <div>
          <a class='readmore' rel="dofollow" href='<%- url_for(post.link || post.path, {relative: false}) %>' itemprop="url">
