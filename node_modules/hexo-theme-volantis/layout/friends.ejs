        ...
        <div class='friends-group'>
            ...
            <div class='friend-content'>
              <% getList(group.items).forEach(function(item){ %>
                <% if (item.url && item.title) { %>
                  <a class='friend-card'
                    target="_blank" rel='external' href="<%- url_for(item.url || '/') %>">
                    <div class='friend-left'>
                      <% if (item.avatar) { %>
                        <img alt="avatar" class='avatar' src='<%- item.avatar || (theme.plugins.lazyload && theme.plugins.lazyload.loadingImg) %>'/>
                      <% } %>
                    </div>
          ...
          <% } else { %>
            <div class='simpleuser-group'>
              <% getList(group.items).forEach(function(item){ %>
                <% if (item.url && item.title) { %>
                  <a class="simpleuser" target="_blank" rel='external' href="<%- url_for(item.url || '/') %>">
                    <img alt="avatar" src="<%- item.avatar || (theme.plugins.lazyload && theme.plugins.lazyload.loadingImg) %>"/>
